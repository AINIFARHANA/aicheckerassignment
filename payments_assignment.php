<?php
session_start();
 $pageTitle = 'Assignment Payment — AI Checker';
...
/* ── ToyyibPay Config ── */
 ...
 $TP_CATEGORY_CODE = isset($assignmentCategoryCode) ? $assignmentCategoryCode : 'j57qrdj8';
 $TP_BASE_URL      = isset($toyyibPayBaseUrl)      ? $toyyibPayBaseUrl      : 'https://dev.toyyibpay.com';
 $ASSIGNMENT_PRICE = 10.00;
 ...
 $returnUrl   = $siteUrl . '/payment_success.php';
 $callbackUrl = $siteUrl . '/payment_success.php?server_callback=1';
...
if (!isset($_GET['id']) || !ctype_digit((string)$_GET['id'])) {
    $_SESSION['flash_error'] = 'Invalid assignment.';
 ...
 $stmt = $conn->prepare("SELECT assignment_id, title, subject FROM assignments WHERE assignment_id = ? AND user_id = ?");
 $stmt->bind_param("ii", $referenceId, $user_id);
 $stmt->execute();
...
if (!$assign) {
 ...
 $itemName       = htmlspecialchars($assign['title']);
 $itemDesc       = 'Assignment check — ' . htmlspecialchars($assign['subject']);
 $itemDuration   = 'One-time';
 $originalAmount = $ASSIGNMENT_PRICE;

/* ── Check active plan discount ── */
 ...
 $subSql = "SELECT us.end_date, p.plan_name, IFNULL(p.assignment_discount,0) as assignment_discount
...
if ($subStmt) {
    ...
    if ($sub) {
        $hasActivePlan       = true;
        $activePlanName      = htmlspecialchars($sub['plan_name']);
        $planDiscountPercent = floatval($sub['assignment_discount']);
        if ($planDiscountPercent > 0) {
            $autoDiscount     = round($originalAmount * ($planDiscountPercent / 100), 2);
            $autoDiscountText = $planDiscountPercent . '% off (active plan: ' . $activePlanName . ')';
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $finalAmount = floatval($input['final_amount'] ?? ($originalAmount - $autoDiscount));
    ...
    $orderId = 'ASG_' . $user_id . '_' . $referenceId . '_' . time();

    /* Defensive caps: ToyyibPay limits billName to ~30 chars and billDescription
       to 100 chars. Title/subject here are user-entered, so cap them so a long
       assignment title/subject can't reproduce the "exceed limit" bill error. */
    $billNameSafe = mb_strlen($itemName) > 30 ? mb_substr($itemName, 0, 30) : $itemName;
    $billDescSafe = mb_strlen($itemDesc) > 100 ? mb_substr($itemDesc, 0, 100) : $itemDesc;

    $billData = [
        'userSecretKey'        => $TP_SECRET_KEY,
        'categoryCode'         => $TP_CATEGORY_CODE,
        'billName'             => $billNameSafe,
        'billDescription'      => $billDescSafe,
        'billPriceSetting'     => 1,
        'billPayorInfo'        => 0,
        'billAmount'           => (string) intval(round($finalAmount * 100)),
    ...
    $totalDiscount = $autoDiscount + $discountAmt;
    ...
    if ($insStmt) {
        $insStmt->bind_param("ississdddss",
            ...
            $originalAmount, $totalDiscount, $finalAmount, $voucherCode, $billCode
        );
        $insStmt->execute();
        $insStmt->close();
    }
...
if (empty($_SESSION['csrf_token'])) {
 ...
 $effectiveTotal = $originalAmount - $autoDiscount;
...
<style>
    ...
    .os-type-badge{padding:4px 14px;background:rgba(34,197,94,0.12);border:1px solid rgba(34,197,94,0.25);border-radius:60px;font-size:0.75rem;font-weight:600;color:#4ade80}
    ...
    .plan-discount-notice{display:flex;align-items:flex-start;gap:10px;padding:14px 16px;margin-bottom:16px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.2);border-radius:12px;font-size:0.82rem;color:#4ade80}
    .plan-discount-notice i{margin-top:2px;font-size:1rem}
    .plan-discount-notice strong{color:#86efac}
...
<section class="payment-section">
    <div class="container">
        ...
        <div class="progress-indicator" data-aos="fade-down">
            <div class="progress-step completed"><div class="step-circle"><i class="fa-solid fa-check" style="font-size:0.8rem"></i></div><span class="step-label">Assignment</span></div>
            <div class="progress-connector filled"></div>
            <div class="progress-step active"><div class="step-circle">2</div><span class="step-label">Payment</span></div>
            <div class="progress-connector"></div>
            <div class="progress-step"><div class="step-circle">3</div><span class="step-label">Success</span></div>
            <div class="progress-connector"></div>
            <div class="progress-step"><div class="step-circle">4</div><span class="step-label">Receipt</span></div>
        </div>

        <div class="text-center mb-5" data-aos="fade-up">
            <div class="secure-badge mb-3"><i class="fa-solid fa-shield-halved me-2"></i>Secure Payment via ToyyibPay</div>
            <h1 class="payment-title">Pay for Assignment Check</h1>
            <p class="payment-subtitle">Complete payment to have your assignment analyzed by AI</p>
        </div>
            ...
            <div class="col-lg-5" data-aos="fade-right" data-aos-delay="100">
                <div class="glass-card order-summary-card">
                    <div class="os-header">
                        ...
                        <span class="os-type-badge">Assignment</span>
                    </div>
                    <div class="os-item">
                        <div class="os-item-img"><div class="os-item-img-placeholder"><i class="fa-solid fa-file-lines"></i></div></div>
                        <div class="os-item-info">
                            <h6><?php echo $itemName; ?></h6>
                            <p><?php echo $itemDesc; ?></p>
                            <span style="display:inline-block;padding:2px 10px;border-radius:60px;font-size:0.7rem;font-weight:600;background:rgba(244,209,255,0.08);border:1px solid rgba(216,143,255,0.2);color:#D88FFF;">One-time Check</span>
                        </div>
                    </div>
                    <?php if ($hasActivePlan && $autoDiscount > 0): ?>
                        <div class="plan-discount-notice"><i class="fa-solid fa-tag"></i><div><strong>Plan Discount Applied!</strong><br><?php echo $autoDiscountText; ?> — You save <strong>RM <?php echo number_format($autoDiscount, 2); ?></strong></div></div>
                    <?php elseif ($hasActivePlan): ?>
                        <div class="plan-discount-notice" style="background:rgba(216,143,255,0.06);border-color:rgba(216,143,255,0.15);color:#D88FFF;"><i class="fa-solid fa-crown"></i><div><strong>Active Plan: <?php echo $activePlanName; ?></strong><br>No assignment discount on this plan.</div></div>
                    <?php endif; ?>
                    <div class="os-divider"></div>
                    <div class="os-pricing">
                        <div class="os-price-row"><span>Check Price</span><span id="originalDisplay"><?php echo $fmtPrice; ?></span></div>
                        <?php if ($autoDiscount > 0): ?>
                        <div class="os-price-row discount-row"><span>Plan Discount (<?php echo $planDiscountPercent; ?>%)</span><span id="planDiscountDisplay">-RM <?php echo number_format($autoDiscount, 2); ?></span></div>
                        <?php endif; ?>
                        <div class="os-price-row discount-row" id="discountRow" style="display:none;"><span>Voucher Discount</span><span id="discountDisplay">-RM 0.00</span></div>
                    </div>
                    <div class="os-divider"></div>
                    <div class="os-total"><span>Total Payment</span><span id="totalDisplay"><?php echo 'RM ' . number_format($effectiveTotal, 2); ?></span></div>
                    <div class="os-secure-note"><i class="fa-solid fa-lock"></i><span>256-bit SSL Encrypted · Powered by ToyyibPay</span></div>
                </div>
            </div>
                ...
                <div class="glass-card payment-form-card">
                    ...
                    <div class="pf-section">
                        ...
                        <div class="pf-user-info">
                            <div class="pf-user-row"><span class="pf-label">Name</span><span class="pf-value"><?php echo htmlspecialchars($userName); ?></span></div>
                            <div class="pf-user-row"><span class="pf-label">Email</span><span class="pf-value"><?php echo htmlspecialchars($userEmail); ?></span></div>
                            <?php if (!empty($userPhone)): ?>
                            <div class="pf-user-row"><span class="pf-label">Phone</span><span class="pf-value"><?php echo htmlspecialchars($userPhone); ?></span></div>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <button type="button" class="btn confirm-payment-btn" id="confirmBtn">
                        ...
                        <span class="btn-amount" id="btnAmount"><?php echo 'RM ' . number_format($effectiveTotal, 2); ?></span>
                        <span class="btn-loading" style="display:none;"><span class="spinner-border spinner-border-sm me-2" role="status"></span>Creating Bill...</span>
                    </button>
                </div>
            </div>
        </div>
...
(function(){
    ...
    var originalPrice         = parseFloat('<?php echo $originalAmount; ?>'),
        planDiscount          = parseFloat('<?php echo $autoDiscount; ?>'),
        currentVoucherDiscount = 0,
        appliedVoucherCode    = '',
        pageUrl               = '<?php echo addslashes($pageUrl); ?>';

    function getBasePrice()    { return originalPrice - planDiscount; }
    function getCurrentTotal() { return Math.max(0, getBasePrice() - currentVoucherDiscount); }
    ...
    function applyVoucher(code) {
        ...
        var body = 'voucher_code=' + encodeURIComponent(code) + '&amount=' + getBasePrice().toFixed(2);
